...
const Conversations = () => {
  ...
  const inferenceState = useAppSelector(state => state.inference);
  ...
  const isSending = selectedThreadId ? !!inferenceState.sendingByThread[selectedThreadId] : false;
  ...
  useEffect(() => {
    if (activeThreadId && selectedThreadId && activeThreadId === selectedThreadId) {
      void dispatch(loadThreadMessages(selectedThreadId));
    }
  }, [activeThreadId, selectedThreadId, dispatch]);
  ...
  const handleSendMessage = async (text?: string) => {
    ...
    chatEventManager.setPendingReaction({
  ...
  const selectedThreadToolTimeline = selectedThreadId
    ? (inferenceState.toolTimelineByThread[selectedThreadId] ?? [])
    : [];
  const selectedInferenceStatus = selectedThreadId
    ? (inferenceState.inferenceStatusByThread[selectedThreadId] ?? null)
    : null;
  const selectedStreamingAssistant = selectedThreadId
    ? (inferenceState.streamingAssistantByThread[selectedThreadId] ?? null)
    : null;
