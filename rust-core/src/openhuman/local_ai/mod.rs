...
impl LocalAiService {
    ...
    pub async fn download_all_models(&self, config: &Config) -> Result<(), String> {
        ...
        let steps = vec![
            ("chat", Self::effective_chat_model_id(config)),
            ("vision", Self::effective_vision_model_id(config)),
            ("embedding", Self::effective_embedding_model_id(config)),
        ];
    ...
    async fn ensure_tts_asset_available(&self, config: &Config) -> Result<(), String> {
        ...
        if let Some(config_url) = config
            ...
            .filter(|v| !v.trim().is_empty())
        {
            let config_dest = PathBuf::from(format!("{}.json", dest.display()));
            let _ = self
...
fn stt_model_target_path(config: &Config) -> PathBuf {
    let id = LocalAiService::effective_stt_model_id(config);
    let path = PathBuf::from(&id);
    if path.is_absolute() {
        path
...
fn tts_model_target_path(config: &Config) -> PathBuf {
    let voice_id = LocalAiService::effective_tts_voice_id(config);
    let path = PathBuf::from(&voice_id);
    if path.is_absolute() {
        return path;
