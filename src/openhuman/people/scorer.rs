...
/// Frequency saturates at this many interactions in the lookback window.
/// 50+ interactions in the window yields frequency = 1.0.
...
pub fn score(interactions: &[Interaction], now: DateTime<Utc>) -> ScoreComponents {
    ...
    // Frequency: total count in the lookback window, saturated.
    let count = interactions.len() as f32;
    let frequency = (count / FREQUENCY_CAP).clamp(0.0, 1.0);

    // Reciprocity: balance of outbound vs inbound — perfect balance = 1.0,
    // all-one-direction = 0.0.
