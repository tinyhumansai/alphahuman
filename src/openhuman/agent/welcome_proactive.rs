...
async fn run_proactive_welcome(config: Config) -> anyhow::Result<()> {
    ...
    // different framing.
    //
    // We also tell the agent that greeting templates have already been
    // shown so it does not open with a redundant "Good morning / Hey
    // there" — the personalised setup summary should start immediately.
    let prompt = format!(
