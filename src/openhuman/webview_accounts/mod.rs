...
//!
//! This module opens the DB read-only and asks a simple question per
//! provider: "is there a row whose `host_key` matches our expected host
//! suffix and whose `name` matches a known session-cookie name?" If so,
//! we report `logged_in: true` for that provider. If the env var is
//! missing, the DB can't be opened (locked, corrupt, nonexistent), or
//! no matching rows exist, we report `logged_in: false` for every
//! provider — never return an error, the welcome-agent snapshot must
//! always build.
//!
...
const COOKIES_DB_ENV: &str = "OPENHUMAN_CEF_COOKIES_DB";
