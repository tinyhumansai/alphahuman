...
impl OpenAiCodexProvider {
    async fn send_responses_request(
        ...
        let access_token = self
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex auth profile not found. Re-authenticate via the web UI."
                )
            })?;
        let account_id = profile
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex account id not found in auth profile/token. Re-authenticate via the web UI."
                )
            })?;
