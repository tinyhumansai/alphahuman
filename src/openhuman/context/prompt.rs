...
impl PromptSection for IdentitySection {
    ...
    fn build(&self, ctx: &PromptContext<'_>) -> Result<String> {
        ...
        // (e.g. LinkedIn scrape). Unlike the builtin files above it has no
        // default content and is never synced — it only exists when the
...
pub fn render_subagent_system_prompt_with_format(
    ...
    //     enrichment pipeline (e.g. LinkedIn scrape). This is NOT part of
    //     the standard identity preamble: agents such as `welcome` set
    //     `omit_identity = true` to drop SOUL/IDENTITY while still needing
    //     PROFILE.md to personalise output. Inject unconditionally — the
    //     injector silently skips when the file is absent, so sub-agents
    //     that run before onboarding has completed pay zero prompt cost.
