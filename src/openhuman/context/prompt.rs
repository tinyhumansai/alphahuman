...
impl PromptSection for IdentitySection {
    ...
    fn build(&self, ctx: &PromptContext<'_>) -> Result<String> {
        ...
        // PROFILE.md is generated by the onboarding enrichment pipeline
        // (e.g. LinkedIn scrape). Unlike the builtin files above it has
        // no default content and is never synced — it only exists when
        // the pipeline has run successfully. Gated by `include_profile`
        // (derived from the agent definition's `omit_profile` flag) so
        // narrow specialists don't pay the token cost.
        if ctx.include_profile {
            inject_workspace_file(&mut prompt, ctx.workspace_dir, "PROFILE.md");
        }
...
pub fn render_subagent_system_prompt_with_format(
    ...
    // 1c. PROFILE.md — user-specific context generated by the onboarding
    //     enrichment pipeline (e.g. LinkedIn scrape). This is NOT part
    //     of the standard identity preamble: agents such as `welcome`
    //     set `omit_identity = true` to drop SOUL/IDENTITY while still
    //     needing PROFILE.md to personalise output. Gated independently
    //     on [`SubagentRenderOptions::include_profile`] (derived from
    //     the agent definition's `omit_profile` flag) so narrow
    //     specialists can skip the token cost.
